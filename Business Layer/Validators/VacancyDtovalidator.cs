...
namespace Business_Layer.Validators
{
    public class VacancyDtovalidator : AbstractValidator<VacancyDto>
    {
        ...
        {
            RuleFor(v => v.Name).NotEmpty().MaximumLength(40);
            RuleFor(v => v.Description).NotEmpty().MaximumLength(500);
            RuleFor(v => v.MaxNumber).NotEmpty().InclusiveBetween(1, 20);
            RuleFor(v => v.ExpiryDate)
                .NotEmpty()
                .Must(FutureDate)
                .WithMessage("the expiration date should be at least 1 day");
        }
